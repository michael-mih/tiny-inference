...
def main():
    ...
    if not args.no_telemetry:
        ...
        print(f"avg_model_latency_ms: {statistics.mean(latencies_ms):.2f}")
        print(f"min_model_latency_ms: {min(latencies_ms):.2f}")
        print(f"max_model_latency_ms: {max(latencies_ms):.2f}")
        ...
        if post_load_vram is None:
            ...
            print(f"peak_vram_during_inference_mb: {max(peak_vram_mb):.2f}")
